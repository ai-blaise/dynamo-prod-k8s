...
// IsIntraPodFailoverEnabled reports whether the component requests the
// intra-pod engine-0/engine-1 failover topology: the main container is
// cloned into two engine containers (active + standby) within the same
// pod, and both engines restore from the same checkpoint.
//
// Inter-pod failover (Mode=interPod) is handled separately via
// expandRolesForService and generatePodSpecForRole and does not use
// container cloning, so its engine pods retain a single main container and
// get the default snapshot-target-containers annotation ("main"). Callers
// sizing up the engine-container list for the snapshot contract therefore
// only need to consult this predicate.
...
// by the intra-pod failover pod transform, in order. Used by the
// checkpoint/restore wiring to stamp the snapshot-target-containers
// annotation so the snapshot agent restores the single checkpoint into
// every engine in the pod.
